...
import { importYugiohCards } from "@/features/cards/services/import-yugioh";
...
export async function importYugioh(offset: number): Promise<ImportResult> {
  const session = await auth();
  if (!session?.user?.id) return { error: "Not authenticated" };
  ...
  const result = await importYugiohCards(offset, 50);
  if (!result.success) return { error: result.error.message };
